...
describe('golfatine 62-88 integration', () => {
  it('has 83 scorecards with official numbering (79 merged into 30)', () => {
    const ids = MATCHES_DATA.map((m) => m.id);
    expect(new Set(ids).size).toBe(ids.length);
    expect(ids).toEqual([...ids].sort((a, b) => a - b));
    expect(GLOBAL_SUMMARY.totalMatches).toBe(83);
    expect(GLOBAL_SUMMARY.totalVideos).toBe(88);
    expect(GLOBAL_SUMMARY.totalScorecards).toBe(386);
    for (const expected of [62, 63, 64, 65, 66, 67, 68, 69, 70, 71, 72, 73, 74, 75, 76, 77, 78, 80, 81, 82, 83, 84, 85, 86, 87, 88]) {
  ...
  it('has full scorecards for 83-84-87-88 (no video-only left)', () => {
    const byId = new Map(MATCHES_DATA.map((m) => [m.id, m]));
    for (const vid of [83, 84, 87, 88]) {
...
describe('timesfm forecast on full dataset', () => {
  it('forecasts match #89 for the 7 main players with coherent probabilities', () => {
    expect(UPCOMING_MATCH_FORECAST.matchNumber).toBe(89);
